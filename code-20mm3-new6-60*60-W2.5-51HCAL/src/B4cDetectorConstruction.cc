  ...
  //
    ...
    for(int i=0;i<num_of_towers;i++){
        ...
        for(int j=0;j<num_of_towers;j++){
            ...
            new G4PVPlacement(0, G4ThreeVector(-45.*cm+i*10.*cm, 45.*cm-j*10.*cm,calorEMZ/2+calorThickness2/2), calorLV[i][j],bu, worldLV, false, 0, fCheckOverlaps);  // no rotation, at (0,0,0), its logical volume,  its name, its mother  volume, no boolean operation, copy number,   checking overlaps
        }
...
calorEM[i][j] = new G4LogicalVolume(calorimeterEM,defaultMaterial,bu);
        new G4PVPlacement(0,G4ThreeVector(-48.75*cm+i*2.5*cm , 48.75*cm-j*2.5*cm ,0.),calorEM[i][j],bu,worldLV,false,0,fCheckOverlaps);
	}
}
