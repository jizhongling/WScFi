...
//	<< "Number of the tower: "<<number1<<number2 
  //   << std::setw(7) << G4BestUnit(absoEdep, "Energy")
   //  << "       total track length: " 
    ...
    // << G4endl
    // << "        Gap: total energy: " 
     G4cout << gapEdep/CLHEP::MeV <<G4endl;
    // << "       total track length: " 
...
{  
  ...
  if ( eventID % fPrintModulo == 0 )  { 
    G4cout << "\n---> Begin of event: " << eventID+1 << G4endl;
...
*/
  ...
  //
	...
	for(int i=0;i<24;i++){
		for(int j=0;j<24;j++){
        		PrintEventStatistics(absoEMHit[i][j]->GetEdep(),absoEMHit[i][j]->GetTrackLength(), absoEMHit[i][j]->GetEdep(), absoEMHit[i][j]->GetTrackLength(),i,j);
