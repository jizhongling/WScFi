...
G4VPhysicalVolume* B4cDetectorConstruction::DefineVolumes()
{
  ...
  G4Element* elW=new G4Element("Tungsten","W",74.,a);
    ...
    G4Material* EMCal_abs_mat=new G4Material("EMCal_fiber_mat", 10.15*g/cm3,2);
    EMCal_abs_mat->AddElement(elW, 94.8*perCent);
    EMCal_abs_mat->AddMaterial(gapMaterial2, 5.2*perCent);
